...
This link of Baidu network disk (百度网盘) includes the full DENA model files (link (百度网盘链接)：**https://pan.baidu.com/s/166FmGUziN91kCLknern6Rw?pwd=ocm8** ; Extracted code(提取码)：**ocm8** ). **Please download the updated model file through the above link, and replace the old model file, and then re-perform RNA m6A modification prediction using the code of step4 (Predict)**. Thanks!
