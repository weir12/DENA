...
class INCEPTION(nn.Module):
	def __init__(self,in_channels,outputs_channels,scale=1):
		...
		self.branch1=nn.Sequential(
			Conv2d(in_channels=in_channels, out_channels=outputs_channels, padding=0,kernel_size=1,stride=1)
			)
		self.branch2=nn.Sequential(
		Conv2d(in_channels=in_channels, out_channels=outputs_channels, padding=0,kernel_size=1,stride=1),
		Conv2d(in_channels=outputs_channels, out_channels=outputs_channels,kernel_size=3,padding=1,stride=1)
		)
		self.branch3=nn.Sequential(
			Conv2d(in_channels=in_channels, out_channels=outputs_channels, padding=0,kernel_size=1,stride=1),
			Conv2d(in_channels=outputs_channels, out_channels=outputs_channels, kernel_size=5,padding=2,stride=1)
			)
		self.branch4=nn.Sequential(
			nn.MaxPool1d(kernel_size=3,padding=1,stride=1),
			Conv2d(in_channels=in_channels, out_channels=outputs_channels, padding=0,kernel_size=1,stride=1)
			)
		self.shortcut=Conv2d(in_channels=in_channels, out_channels=outputs_channels*4,kernel_size=1,stride=1,padding=0)
	def forward(self, x):
		out1 = self.branch1(x)
		out2 = self.branch2(x)
		out3 = self.branch3(x)
