...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (RNN_data, target) in enumerate(self.data_loader):
            RNN_data,target = RNN_data.to(self.device),target.to(self.device)

            self.optimizer.zero_grad()
            output = self.model(RNN_data)
            ...
            if batch_idx % self.log_step == 0:
                self.logger.debug('Train Epoch: {} {} Loss: {:.6f}'.format(
                    ...
                    loss.item()))
                #self.writer.add_image('input', make_grid(RNN_data.cpu(), nrow=8, normalize=True))
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (RNN_data, target) in enumerate(self.valid_data_loader):
                RNN_data,target = RNN_data.to(self.device),target.to(self.device)
